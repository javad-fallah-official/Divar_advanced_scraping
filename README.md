...
This project scrapes listings from Divar for Tehran city, focusing on `motorcycles` → `honda` and filtering out negotiable ads (`non-negotiable=true`). It extracts structured data from each product page and saves it into a PostgreSQL database.
...
- Saves to PostgreSQL with upsert (deduplicates by `url`)
...
Output DB: PostgreSQL (configure via environment variables `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`, `DB_NAME`).
...
A separate project using the unofficial `divar` PyPI library can be evaluated independently; this scraper stores directly to PostgreSQL for performance and reliability.
